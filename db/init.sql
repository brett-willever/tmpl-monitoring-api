...
CREATE OR REPLACE FUNCTION update_timestamps() RETURNS TRIGGER AS $$ BEGIN NEW.updated_at = NOW();
NEW.last_activity_at = NOW();
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
-- Create a trigger to automatically update the timestamps
CREATE TRIGGER trigger_update_timestamps BEFORE
INSERT
    OR
UPDATE ON apps FOR EACH ROW EXECUTE FUNCTION update_timestamps();
-- Function to remove inactive apps after 30 days
CREATE OR REPLACE FUNCTION remove_inactive_apps() RETURNS VOID AS $$ BEGIN
DELETE FROM apps
...
CREATE OR REPLACE FUNCTION remove_all_finished_apps() RETURNS VOID AS $$ BEGIN
DELETE FROM apps;
END;
$$ LANGUAGE plpgsql;
-- Wrapper function to decide which action to take
CREATE OR REPLACE FUNCTION cleanup(criteria VARCHAR(255)) RETURNS VOID AS $$ BEGIN IF criteria = 'inactive_after_30_days' THEN PERFORM remove_inactive_apps();
ELSIF criteria = 'remove_all' THEN PERFORM remove_all_finished_apps();
ELSE RAISE EXCEPTION 'Invalid criteria provided';
END IF;
END;
$$ LANGUAGE plpgsql;
